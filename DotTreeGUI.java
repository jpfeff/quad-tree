...
public class DotTreeGUI extends DrawingGUI {
	...
	private void test2() {
		found = null;
		tree = new PointQuadtree<Dot>(new Dot(400,200), 0,0,800,600); // start with A
		tree.insert(new Dot(200,125)); // B
		tree.insert(new Dot(300,350)); // C
		tree.insert(new Dot(150,475)); // D
		tree.insert(new Dot(575,525)); // E
		tree.insert(new Dot(680,380)); // F
		tree.insert(new Dot(615,120)); // G
		tree.insert(new Dot(670,160)); // H
		int bad = 0;
		bad += testFind(640,140,50,6,3,2);
		bad += testFind(400,200,1,8,5,1);
		bad += testFind(290,360,20,6,3,1);
		bad += testFind(0,0,900,8,8,8);
	...
	public void drawTree(Graphics g, PointQuadtree<Dot> tree, int level) {
		...
		g.fillOval((int)(tree.getPoint().getX()-dotRadius), (int)(tree.getPoint().getY()-dotRadius), (int)(2*dotRadius), (int)(2*dotRadius));

		g.drawLine((int)tree.getPoint().getX(),tree.getY1(),(int)tree.getPoint().getX(), tree.getY2());
		g.drawLine((int)tree.getX1(),(int)tree.getPoint().getY(), tree.getX2(),(int)tree.getPoint().getY());

		// Recurse with children
		// TODO: YOUR CODE HERE
		if (tree.hasChild(1)){
			// increment level to change color
			level++;
